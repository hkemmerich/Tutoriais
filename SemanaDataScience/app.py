...
st.subheader("Seecionando apenas um pequeno conjunto de artibutos")

# atributos para serem exibidos por padrão
defaultcols = ["RM", "PTRATIO", "LSTAT", "MEDV"]

# definindo atributos a partir do mulselect
cols = st.multiselect("Artibutos", data.columns.tolist(), default = defaultcols)
...
indus = st.sidebar.number_input("Proporção de Hectar de Negócios", value=data.INDUS.mean())
chas = st.sidebar.selectbox("Faz limite com rio?", ("Sim", "Não"))

#transfomando o dado de entrada em calor binário
chas = 1 if chas == "Sim" else 0

nox = st.sidebar.number_input("Concentração de óxido nítrico", value=data.NOX.mean())

rm = st.sidebar.number_input("Números de quartos", value=1)
...
lstat = st.sidebar.number_input("Porcentage, de status baixo", value= data.LSTAT.mean())
...
if btn_predict:
    ...
    st.subheader("Ovalor previsto para o imóvel é:")
